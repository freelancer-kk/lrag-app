...
export default class SemanticChunking {
  ...
  splitToSentences = async (textCorpus: string,  params: Partial<RecursiveCharacterTextSplitterParams> | undefined, language: LanguageTypes | undefined): Promise<string[]> => {
    let splitter;
  ...
  chunk = async (model: string, doc: Document, chunkNum: number, totalChunks: number): Promise<Document[]> => {
    try {
      ...
      const sentences: string[] = this.splitToSentencesUsingNLP(doc.pageContent);

      // Step 3: Structure these sentences into an array of SentenceObject.
      const structuredSentences: SentenceObject[] = this.structureSentences(sentences, 1); // Assuming a bufferSize of 1 for simplicity

      // Step 4: Generate embeddings for these combined sentences.
      const sentencesWithEmbeddings: SentenceObject[] = await this.generateAndAttachEmbeddings(
        structuredSentences
      );
      ...
      const { updatedArray, significantShiftIndices } =
        this.calculateCosineDistancesAndSignificantShifts(sentencesWithEmbeddings, 90); // Assuming a threshold of 90%

      // Step 6: Group sentences into semantic chunks based on the significant shifts identified.
      const semanticChunks: string[] = this.groupSentencesIntoChunks(
        updatedArray,
        significantShiftIndices
      );
